...
class MarineBot(sc2.BotAI):
    ...
    def calc_scores(self):
        ...
        # first_vision = self.state.visibility.data_numpy.copy()
        # first_vision[first_vision == 2] = "X"
        # self.visibility_scores = 0

        # for y in range(len(vgrid)):
        #     for x in range(len(vgrid[0])):
        #         if l_grid.is_set((x, y)):
        #             np.put(l_grid[y], [x], [1])
        #         else:
        #             np.put(l_grid[y], [x], [0])

        # for r in self.visibility_scores:
        #     print(r)
